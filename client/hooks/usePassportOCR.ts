...
export function usePassportOCR() {
  ...
  const extractPassportData = async (
    file: File,
  ): Promise<ExtractedPassportData | null> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (responseData?.code === "BILLING_NOT_ENABLED") {
          errorMessage =
            "Google API billing not fully enabled yet. Please wait 5-10 minutes and try again.";
        } else if (responseData?.code === "CREDENTIAL_ERROR") {
          errorMessage = "Server configuration error. Please contact support.";
        } else if (responseData?.code === "INVALID_FORMAT") {
          errorMessage =
            "Invalid image format. Please upload a clear JPG, PNG, or PDF.";
      ...
      if (result.confidence >= 0.7) {
        ...
        toast.warning(
          `Low confidence (${Math.round(result.confidence * 100)}%). Please verify.`,
        );
      } else {
