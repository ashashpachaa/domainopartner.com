...
export default function AdminAttendanceReport() {
  ...
  const calculateStats = () => {
    ...
    Object.keys(stats).forEach((staffId) => {
      const total =
        stats[staffId].confirmations + stats[staffId].missedConfirmations;
      stats[staffId].attendanceRate =
        total > 0
          ? Math.round((stats[staffId].confirmations / total) * 100)
          : 0;
    });
          ...
          <div className="bg-white rounded-lg p-6 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900">
                  {globalStats.totalHours.toFixed(1)}
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-3xl font-bold ${globalStats.avgAttendanceRate >= 90 ? "text-green-600" : globalStats.avgAttendanceRate >= 70 ? "text-yellow-600" : "text-red-600"}`}
                >
              ...
              <tbody className="divide-y divide-slate-200">
                {stats.map((stat, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-6 py-4 text-center text-slate-700">
                      {stat.daysWorked > 0
                        ? (stat.totalHours / stat.daysWorked).toFixed(1)
                        : 0}{" "}
                      hrs
                    </td>
            ...
            onClick={() => {
              const csv = [
                ...
                ...stats.map((s) => [
                  ...
                  (s.daysWorked > 0 ? s.totalHours / s.daysWorked : 0).toFixed(
                    1,
                  ),
                  s.confirmations,
