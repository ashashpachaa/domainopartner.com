    ...
    <ClientLayout>
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-6">
            <p className="text-sm font-medium text-slate-600 uppercase">Renewal in 90 Days</p>
        ...
        {sortedCompanies.length === 0 ? (
          <div className="bg-white rounded-lg border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-600 text-lg font-medium">No registered companies yet</p>
            <p className="text-slate-500 mt-2">
              Create your first company through our order form to see it appear here
            </p>
            <Button
          ...
          <div className="space-y-4">
            {sortedCompanies.map((company) => {
              ...
              const isAccountsSoon = daysUntilAccounts <= 90 && daysUntilAccounts > 0;
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-slate-500 mt-1">Company No: {company.companyNumber}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    <div>
                      <p className="text-xs text-slate-600 uppercase font-semibold mb-1">Incorporated</p>
                      <p className="font-medium text-slate-900">{formatDate(company.incorporationDate)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-600 uppercase font-semibold mb-1">Next Renewal</p>
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-slate-900">{formatDate(company.nextRenewalDate)}</p>
                        {isRenewalSoon && (
                          <AlertCircle className="w-4 h-4 text-orange-600" title={`${daysUntilRenewal} days remaining`} />
                        )}
                      </div>
                      {isRenewalSoon && (
                        <p className="text-xs text-orange-600 mt-1">{daysUntilRenewal} days left</p>
                      )}
                    </div>
                    <div>
                      <p className="text-xs text-slate-600 uppercase font-semibold mb-1">Accounts Due</p>
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-slate-900">
                          {formatDate(company.nextAccountsFilingDate)}
                        </p>
                        {isAccountsSoon && (
                          <AlertCircle className="w-4 h-4 text-orange-600" title={`${daysUntilAccounts} days remaining`} />
                        )}
                      </div>
                      {isAccountsSoon && (
                        <p className="text-xs text-orange-600 mt-1">{daysUntilAccounts} days left</p>
                      )}
                    </div>
                    <div>
                      <p className="text-xs text-slate-600 uppercase font-semibold mb-1">Data Retrieved</p>
                      <p className="font-medium text-slate-900">{formatDate(company.fetchedAt)}</p>
                    </div>
                  </div>

                  {company.registeredOffice && (
                    <div className="mb-4 pt-4 border-t border-slate-200">
                      <p className="text-xs text-slate-600 uppercase font-semibold mb-1">Registered Office</p>
                      <p className="text-sm text-slate-700">{company.registeredOffice}</p>
                    </div>
                  )}
